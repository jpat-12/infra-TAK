...
function _vacuumSetUI(running,full,secs){var msg=document.getElementById('vacuum-msg');var btnA=document.getElementById('vacuum-analyze-btn');var btnF=document.getElementById('vacuum-full-btn');var btnR=document.getElementById('reindex-btn');if(btnA)btnA.disabled=running;if(btnF)btnF.disabled=running;if(btnR)btnR.disabled=running;if(running&&msg){var elapsed='';if(secs>0){var m=Math.floor(secs/60);elapsed=m>0?' ('+m+'m '+secs%60+'s)':' ('+secs+'s)';}msg.textContent=(full?'Running VACUUM FULL':'Running VACUUM ANALYZE')+'...'+elapsed;msg.style.color='var(--text-dim)';}}
