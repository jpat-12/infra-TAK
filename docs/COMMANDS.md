...
git commit -m "v0.3.1-alpha"
git push origin main
git tag v0.3.1-alpha && git push origin v0.3.1-alpha
git checkout dev
```

**Note:** If a file doesn’t exist on dev (e.g. you removed `scripts/fix-mediamtx-stream-redirect.sh`), drop that line from the `git checkout dev --` list. For a new release, change the release doc (e.g. `docs/RELEASE-v0.3.1-alpha.md` → next release), the commit message, and the tag; then run the tag push.
