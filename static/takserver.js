...
if(document.getElementById('cot-db-size')){refreshCotSize();fetch('/api/takserver/vacuum/status').then(function(r){return r.json();}).then(function(d){if(d.running){window._vacuumStarted=d.started?new Date(d.started).getTime():Date.now();_vacuumSetUI(true,d.full);_pollVacuumStatus();}}).catch(function(){});}
...
function _vacuumSetUI(running,full){var msg=document.getElementById('vacuum-msg');var btnA=document.getElementById('vacuum-analyze-btn');var btnF=document.getElementById('vacuum-full-btn');var btnR=document.getElementById('reindex-btn');if(btnA)btnA.disabled=running;if(btnF)btnF.disabled=running;if(btnR)btnR.disabled=running;if(running&&msg){var started=window._vacuumStarted;var elapsed='';if(started){var s=Math.round((Date.now()-started)/1000);var m=Math.floor(s/60);elapsed=m>0?' ('+m+'m '+s%60+'s)':' ('+s+'s)';}msg.textContent=(full?'Running VACUUM FULL':'Running VACUUM ANALYZE')+'...'+elapsed;msg.style.color='var(--text-dim)';}}
async function _pollVacuumStatus(){var msg=document.getElementById('vacuum-msg');var out=document.getElementById('vacuum-output');try{var r=await fetch('/api/takserver/vacuum/status');var d=await r.json();if(d.running){_vacuumSetUI(true,d.full);setTimeout(_pollVacuumStatus,5000);}else{if(d.error){if(msg){msg.textContent=d.error;msg.style.color='var(--red)';}if(out){out.textContent=d.error;out.style.display='block';}}else if(d.result!==null){if(msg){msg.textContent='Done.';msg.style.color='var(--green)';}if(out&&d.result){out.textContent=d.result;out.style.display='block';}refreshCotSize();}window._vacuumStarted=null;_vacuumSetUI(false,false);}}catch(e){setTimeout(_pollVacuumStatus,10000);}}
async function runVacuum(full){var msg=document.getElementById('vacuum-msg');var out=document.getElementById('vacuum-output');if(full&&!confirm("VACUUM FULL locks the CoT tables. Run when TAK Server is not running. Continue?"))return;if(out){out.style.display='none';out.textContent='';}window._vacuumStarted=Date.now();_vacuumSetUI(true,full);try{var r=await fetch('/api/takserver/vacuum',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({full:full})});var d=await r.json();if(d.success||d.status==='running'){_pollVacuumStatus();}else{if(msg){msg.textContent=d.error||'Failed';msg.style.color='var(--red)';}window._vacuumStarted=null;_vacuumSetUI(false,false);}}catch(e){if(msg){msg.textContent='Error: '+e.message;msg.style.color='var(--red)';}window._vacuumStarted=null;_vacuumSetUI(false,false);}}
