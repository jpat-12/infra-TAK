    ...
    <div class="gd-section">
      ...
      <div id="gd-sms-brevo" style="display:{{ 'block' if guarddog_sms.get('provider')=='brevo' else 'none' }};margin-bottom:10px">
        ...
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
          <input class="form-input" type="text" id="gd-sms-br-sender" placeholder="Sender (max 11 chars)" value="{{ guarddog_sms.get('sender','') }}" style="flex:1;margin-bottom:0" maxlength="11" oninput="gdSenderCheck()">
...
if(!d.running){clearInterval(gdLogInterval);var btn=document.getElementById('gd-deploy-btn');if(btn)btn.disabled=false;if(d.complete){if(logEl)logEl.textContent+=String.fromCharCode(10,10)+'Deploy complete. Refreshing...';setTimeout(function(){location.reload();},2000);}}});}poll();gdLogInterval=setInterval(poll,800);}
