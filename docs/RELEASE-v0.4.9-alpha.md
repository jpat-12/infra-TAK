...
- Node-RED `docker-compose.yml` is patched in-place and the container restarted
- Authentik `docker-compose.yml` port bindings are patched in-place
- Authentik is then restarted via the normal reconfigure flow
...
- [ ] `app.py` → `VERSION = "0.4.9-alpha"` matches tag **`v0.4.9-alpha`**.
- [ ] Tag **`v0.4.9-alpha`** and push after `main` push.
