...
const engineFlows = [
  ...
  // ════════════════════════════════════════════════
  {
    ...
    func: [
      ...
      "      var simplified = [];",
      "      for (var s = 0; s < MAX_VERTS; s++) simplified.push(ring[Math.floor(s * step)]);",
      "      if (ring[ring.length-1]) simplified.push(ring[ring.length-1]);",
      "      ring = simplified;",
      "    }",
  ...
  },
  {
    ...
    func: [
      ...
      "}",
      "",
      "// ONE batched PUT for all new UIDs (instead of N individual requests)",
      "if (newUids.length > 0) {",
      "  node.send([",
  ...
  },
  {
    ...
    name: 'Wait 60s for CoT ingest', pauseType: 'delay', timeout: '60', timeoutUnits: 'seconds',
