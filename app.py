...
<div class="main">
  ...
  <div class="card"><div class="card-title">Database maintenance (CoT)</div>
    ...
    <div style="display:flex;flex-wrap:wrap;gap:10px;align-items:center">
      <button type="button" id="gd-vacuum-analyze-btn" onclick="gdRunVacuum(false)" class="btn" style="background:linear-gradient(135deg,#1e40af,#0e7490);color:#fff">Run VACUUM ANALYZE</button>
      <button type="button" id="gd-vacuum-full-btn" onclick="gdRunVacuum(true)" class="btn btn-ghost" style="color:var(--yellow);border-color:var(--yellow)" title="Caution: stop TAK Server first or run during a maintenance window">VACUUM FULL</button> <span style="font-size:11px;color:var(--yellow)">(yellow = caution)</span>
    </div>
    <p style="font-size:11px;color:var(--text-dim);margin-top:10px;margin-bottom:0"><strong>VACUUM ANALYZE</strong> — Reclaims space from deleted rows. Safe while TAK is running. &nbsp; <strong>VACUUM FULL</strong> — Locks tables; <strong>stop TAK Server first</strong> or run during a maintenance window.</p>
...
<button type="button" id="vacuum-full-btn" onclick="runVacuum(true)" style="padding:10px 20px;background:rgba(234,179,8,0.2);color:var(--yellow);border:1px solid var(--border);border-radius:8px;font-family:'DM Sans',sans-serif;font-size:13px;font-weight:600;cursor:pointer" title="Caution: stop TAK Server first or run during a maintenance window">VACUUM FULL</button> <span style="font-size:11px;color:var(--yellow)">(yellow = caution)</span>
</div>
<p style="font-size:11px;color:var(--text-dim);margin-top:10px;margin-bottom:0"><strong>VACUUM ANALYZE</strong> — Reclaims space from deleted rows and updates statistics. Safe to run while TAK is running.<br><strong>VACUUM FULL</strong> — Rewrites tables to reclaim more space; <strong>locks tables</strong>. <strong>Stop TAK Server first</strong> or run during a maintenance window.</p>
