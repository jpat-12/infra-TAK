...
3. **Password:** The password that works for 8446 is the one **in Authentik** for `webadmin`. It is set from the TAK Server deploy password when you run Connect/Resync LDAP or **Sync webadmin to Authentik**. To change it, use **Set webadmin password** on the TAK Server page (or set in Authentik and then Sync webadmin to update the server’s stored password). After changing, restart the LDAP outpost on the Authentik host if needed (Resync LDAP does this).
